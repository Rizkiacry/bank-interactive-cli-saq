...
#figure(
  image("/media/41-implementasi-struct.png", width: 80%),
  caption: [Implementasi Struktur Data BankAccount.]
)
...
#figure(
  image("/media/421-implementasi-array.png", width: 80%),
  caption: [Implementasi Array Statis untuk Akun Bank.]
)
...
#figure(
  image("/media/422-akses-array.png", width: 80%),
  caption: [Akses Elemen Array Menggunakan Subscript Operator.]
)
...
#figure(
  image("/media/423-iterasi-array.png", width: 80%),
  caption: [Iterasi Array untuk Inisialisasi Akun.]
)
...
#figure(
  image("/media/431-implementasi-deposit.png", width: 80%),
  caption: [Implementasi Fungsi `deposit()` dengan Parameter Referensi.]
)
...
#figure(
  image("/media/432-implementasi-modify.png", width: 80%),
  caption: [Implementasi Fungsi `modify_account()`.]
)
...
#figure(
  image("/media/441-include-iostream.png", width: 80%),
  caption: [Deklarasi Header `<iostream>`.]
)

Operator ekstraksi `>>` membaca data dari input stream:

#figure(
  image("/media/442-implementasi-cin.png", width: 80%),
  caption: [Penggunaan Operator Ekstraksi `cin >>`.]
)

Untuk membaca string dengan spasi, digunakan `getline()`:

#figure(
  image("/media/443-implementasi-getline.png", width: 80%),
  caption: [Penggunaan `getline()` untuk Input String dengan Spasi.]
)

Operator insersi `<<` menampilkan data ke output stream:

#figure(
  image("/media/444-implementasi-cout.png", width: 80%),
  caption: [Penggunaan Operator Insersi `cout <<`.]
)
...
#figure(
  image("/media/45-implementasi-switch.png", width: 80%),
  caption: [Implementasi `switch-case` untuk Menu Utama.]
)
...
#figure(
  image("/media/461-implementasi-pencarian.png", width: 80%),
  caption: [Implementasi Fungsi `find_account_idx()` (Pencarian Linear).]
)
...
#figure(
  image("/media/462-penggunaan-pencarian.png", width: 80%),
  caption: [Penggunaan Fungsi `find_account_idx()`.]
)
...
#figure(
  image("/media/471-util-is-empty.png", width: 80%),
  caption: [Implementasi Fungsi `is_account_empty()`.]
)
...
#figure(
  image("/media/472-util-find-slot.png", width: 80%),
  caption: [Implementasi Fungsi `find_available_slot()`.]
)
...
#figure(
  image("/media/473-util-initialize.png", width: 80%),
  caption: [Implementasi Fungsi `initialize_account()`.]
)
