...
Struktur data dalam C++ didefinisikan menggunakan keyword `struct` untuk mengelompokkan beberapa variabel dari tipe data yang berbeda menjadi satu kesatuan logis. Menurut dokumentasi @cppreference-struct dan @geeksforgeeks-structures, struct dalam C++ merupakan user-defined type yang mengkombinasikan multiple members dengan tipe berbeda dalam suatu composite type. Ketika struct dideklarasikan dengan body, pernyataan tersebut sekaligus merupakan definisi yang mengalokasikan template tipe dalam sistem type C++.

Dalam proyek ini, struktur `BankAccount` digunakan untuk merepresentasikan data tunggal akun:

#figure(
  image("../../media/21-struct-data.png", width: 80%),
  caption: [Struktur data BankAccount.]
)
...
Array merupakan koleksi elemen tipe data yang sama, tersimpan dalam lokasi memori yang berdampingan (contiguous) @geeksforgeeks-arrays. Dalam C++, deklarasi array menggunakan notasi `type name[size]`, di mana `size` merupakan konstanta yang menentukan jumlah maksimal elemen.

Menurut @cppreference-subscript dan @microsoft-learn-subscript, subscript operator `[]` digunakan untuk mengakses elemen array pada indeks tertentu. Operator ini didefinisikan secara identik dengan ekspresi pointer arithmetic: `E1[E2]` secara fundamental equivalent dengan `*((E1)+(E2))`. Indeksasi array dalam C++ dimulai dari 0, sehingga array dengan size `n` memiliki indeks valid dari 0 hingga `n-1`.

Dalam proyek, array `BankAccount accounts[MAX_ACCOUNTS]` menyimpan hingga lima akun:

#figure(
  image("../../media/22-array-index.png", width: 80%),
  caption: [Array `BankAccount accounts[MAX_ACCOUNTS]`.]
)
...
Fungsi dalam C++ merupakan blok kode yang dapat dipanggil berulang dengan identitas dan tipe parameter tertentu. Dokumentasi @cppreference-function menyatakan bahwa function declaration memperkenalkan nama fungsi dan tipe fungsi, sedangkan function definition mengasosiasikan nama/tipe dengan body fungsi.
...
#figure(
  image("../../media/23-fungsi-referensi.png", width: 80%),
  caption: [Contoh Fungsi dengan Parameter Referensi.]
)
...
Stream dalam C++ merupakan abstraksi untuk komunikasi data antara program dan external device. Menurut dokumentasi @cppreference-iostream dan @geeksforgeeks-io, iostream header mendefinisikan object global `cin` dengan tipe `istream` untuk membaca input dari keyboard, dan `cout` dengan tipe `ostream` untuk menampilkan output ke layar.

Ekstraksi data dari stream menggunakan operator `>>`: @w3schools-cin

#figure(
  image("../../media/241-cin.png", width: 80%),
  caption: [Ekstraksi data dari stream menggunakan operator `>>` (cin).]
)

Insersi data ke stream menggunakan operator `<<`:

#figure(
  image("../../media/242-cout.png", width: 80%),
  caption: [Insersi data ke stream menggunakan operator `<<` (cout).]
)
...
#figure(
  image("../../media/243-getline.png", width: 80%),
  caption: [Penggunaan `cin.ignore()` dan `getline()`.]
)
...
#figure(
  image("../../media/25-switch-case.png", width: 80%),
  caption: [Struktur Kontrol Alur `switch-case`.]
)
...
#figure(
  image("../../media/26-pencarian-linear.png", width: 80%),
  caption: [Implementasi Algoritma Pencarian Linear.]
)
